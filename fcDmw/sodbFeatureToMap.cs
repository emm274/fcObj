...
namespace fcDmw
{
	...
	public class sodbFeatureToMap: sodbFeatureToIntf
	{
		...
		bool obj_shape(sodb db, List<SOAPService.Metadata> meta, ref int count, ref int offset, ref ulong pos)
		{
			if (__obj(ref count))
			{
				if (count == 1)
		...
		public void Features(sodb db, List<SOAPService.Feature> list)
		{
			...
			if (fe.IsRealFeature)
            {
                ...
                ulong __pos = fmap.Position;

                if (rgn != null && rgn.Rings.Count > 0)
                {
					obj.Loc = 3;
					...
					obj_shape(db,rgn.Metadata, ref __count, ref __offs, ref __pos);
				}

				if (ln != null && ln.Paths.Count > 0)
                {
					obj.Loc = 2;
					if (__count > 0) { 
					...
					obj_shape(db,ln.Metadata, ref __count, ref __offs, ref __pos);
				}
				...
				if (pt != null)
                {
					...
					obj_shape(db,pt.Metadata, ref __count, ref __offs, ref __pos);
				}
